...
impl<const TYPE: u8, const VARIANT: usize> Hash for Keccak<TYPE, VARIANT> {
    ...
    fn hash(&self, input: &[Self::Input]) -> Result<Self::Output> {
        let result = match (TYPE, VARIANT) {
            (0, 224) => {
                bits_from_bytes_le(&keccak_224_native(&bytes_from_bits_le(input))).collect()
            }
            (0, 256) => {
                bits_from_bytes_le(&keccak_256_native(&bytes_from_bits_le(input))).collect()
            }
            (0, 384) => {
                bits_from_bytes_le(&keccak_384_native(&bytes_from_bits_le(input))).collect()
            }
            (0, 512) => {
                bits_from_bytes_le(&keccak_512_native(&bytes_from_bits_le(input))).collect()
            }
