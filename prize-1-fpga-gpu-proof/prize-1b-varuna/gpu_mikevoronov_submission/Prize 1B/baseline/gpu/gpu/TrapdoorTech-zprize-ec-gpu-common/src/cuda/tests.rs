...
mod test_multiexp {
    ...
    pub fn test_gpu_multiexp() {
        ...
        let precalc_container =
            MsmPrecalcContainer::create_with_core(&GPU_CUDA_CORES[GPU_DEV_IDX], false).unwrap();
    ...
    pub fn test_gpu_precalc_ed() {
        ...
        let (acc_points_3, _) = kern
            .setup_ed_base(GPU_OP::SETUP_SHIFTED_LAGRANGE_G)
            .unwrap();

        kern.setup_ed_acc_points(GPU_OP::REUSE_G, &acc_points_1, window_size)
            .unwrap();
        kern.setup_ed_acc_points(GPU_OP::REUSE_SHIFTED_G, &acc_points_2, window_size)
            .unwrap();
        kern.setup_ed_acc_points(GPU_OP::REUSE_SHIFTED_LAGRANGE_G, &acc_points_3, window_size)
            .unwrap();
