...
impl<E: PairingEngine> MultilinearPC<E> {
    ...
    pub fn commit(
        ...
        let scalars: Vec<_> = polynomial
            ...
            .collect();
        let g_product =
            ponos_engine::run_msm(&ck.powers_of_g[0], scalars.as_slice()).into_affine();
    ...
    pub fn open(
        ...
        for i in 0..nv {
            ...
            let pi_h =
                ponos_engine::run_msm(&ck.powers_of_h[i], &scalars).into_affine(); // no need to move outside and partition
            proofs.push(pi_h);
        }
