...
impl<E: PairingEngine> CommitterKey<E> {
    ...
    /// Obtain powers for the underlying KZG10 construction
    pub fn ext_powers<'a>(&'a self) -> kzg10::ExtPowers<'a, E> {
        kzg10::ExtPowers {
            powers_of_g: self.powers.as_slice().into(),
            n_powers_of_g: ponos_engine::to_normal_vec(self.powers.as_slice()).into(),
            powers_of_gamma_g: self.powers_of_gamma_g.as_slice().into(),
            n_powers_of_gamma_g: ponos_engine::to_normal_vec(self.powers_of_gamma_g.as_slice()).into(),
        }
    }
    ...
    /// Obtain powers for committing to shifted polynomials.
    pub fn ext_shifted_powers<'a>(
        &'a self,
        degree_bound: impl Into<Option<usize>>,
    ) -> Option<kzg10::ExtPowers<'a, E>> {
        self.shifted_powers.as_ref().map(|shifted_powers| {
            let powers_range = if let Some(degree_bound) = degree_bound.into() {
                assert!(self
                    .enforced_degree_bounds
            ...
            let ck = kzg10::ExtPowers {
                powers_of_g: (&shifted_powers[powers_range.clone()]).into(),
                n_powers_of_g: ponos_engine::to_normal_vec(&shifted_powers[powers_range.clone()]).into(),
                powers_of_gamma_g: self.powers_of_gamma_g.as_slice().into(),
                n_powers_of_gamma_g: ponos_engine::to_normal_vec(self.powers_of_gamma_g.as_slice()).into(),
            };
            ck
