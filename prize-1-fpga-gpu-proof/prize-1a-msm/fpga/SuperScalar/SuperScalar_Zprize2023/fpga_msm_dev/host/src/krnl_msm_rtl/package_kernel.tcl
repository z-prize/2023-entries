...
set path_to_hdl "./src/krnl_msm_rtl/hdl"
...
add_files -norecurse [glob $path_to_hdl/*.v $path_to_hdl/*.sv $path_to_hdl/msm/*.v]

add_files -norecurse $path_to_hdl/basic.vh $path_to_hdl/env.vh
add_files -norecurse [glob $path_to_hdl/*.vh]

set_property file_type SystemVerilog [get_files *.v]
set_property top krnl_vadd_rtl [current_fileset]

if { $zprize_mul_mod == "fake" } {
set_property verilog_define {FPGA FPGA_CELL CFG_UIART_PLL=26 SINGLE_XTAL PROJECT_CR190 PROJECT_CR190_SERIAL ASCII VU9P UNIT_TEST_CALC_BUCKET NEW_BUCKETCONTROL ZPRIZE_FAKE_MUL_MOD} [current_fileset]
} else {
set_property verilog_define {FPGA FPGA_CELL CFG_UIART_PLL=26 SINGLE_XTAL PROJECT_CR190 PROJECT_CR190_SERIAL ASCII VU9P UNIT_TEST_CALC_BUCKET NEW_BUCKETCONTROL} [current_fileset]
}
...
ipx::associate_bus_interfaces -busif m_axi_gmem3 -clock ap_clk $core
ipx::associate_bus_interfaces -busif m_axi_gmem4 -clock ap_clk $core
...
set reg      [::ipx::add_register -quiet "in0" $addr_block]
  set_property address_offset 0x010 $reg
  set_property size           [expr {8*8}]   $reg
  set regparam [::ipx::add_register_parameter -quiet {ASSOCIATED_BUSIF} $reg] 
  set_property value m_axi_gmem0 $regparam 

set reg      [::ipx::add_register -quiet "in1_0" $addr_block]
  set_property address_offset 0x01C $reg
  set_property size           [expr {8*8}]   $reg
  set regparam [::ipx::add_register_parameter -quiet {ASSOCIATED_BUSIF} $reg] 
  set_property value m_axi_gmem1 $regparam 

set reg      [::ipx::add_register -quiet "in1_1" $addr_block]
  set_property address_offset 0x028 $reg
  set_property size           [expr {8*8}]   $reg
  set regparam [::ipx::add_register_parameter -quiet {ASSOCIATED_BUSIF} $reg] 
  set_property value m_axi_gmem2 $regparam 

set reg      [::ipx::add_register -quiet "in2" $addr_block]
  set_property address_offset 0x034 $reg
  set_property size           [expr {8*8}]   $reg
  set regparam [::ipx::add_register_parameter -quiet {ASSOCIATED_BUSIF} $reg] 
  set_property value m_axi_gmem3 $regparam 

set reg      [::ipx::add_register -quiet "in3" $addr_block]
  set_property address_offset 0x040 $reg
  set_property size           [expr {8*8}]   $reg
  set regparam [::ipx::add_register_parameter -quiet {ASSOCIATED_BUSIF} $reg] 
  set_property value m_axi_gmem4 $regparam 

set reg      [::ipx::add_register -quiet "out0" $addr_block]
  set_property address_offset 0x04c $reg
  set_property size           [expr {8*8}]   $reg
  set regparam [::ipx::add_register_parameter -quiet {ASSOCIATED_BUSIF} $reg] 
  set_property value m_axi_gmem0 $regparam 

# set reg      [::ipx::add_register -quiet "out1" $addr_block]
#   set_property address_offset 0x058 $reg
#   set_property size           [expr {8*8}]   $reg
#   set regparam [::ipx::add_register_parameter -quiet {ASSOCIATED_BUSIF} $reg] 
#   set_property value m_axi_gmem1 $regparam 

set reg      [::ipx::add_register -quiet "out2" $addr_block]
  set_property address_offset 0x064 $reg
  set_property size           [expr {8*8}]   $reg
  set regparam [::ipx::add_register_parameter -quiet {ASSOCIATED_BUSIF} $reg] 
  set_property value m_axi_gmem3 $regparam 

set reg      [::ipx::add_register -quiet "out3" $addr_block]
...
set reg      [::ipx::add_register -quiet "in1_2" $addr_block]
  set_property address_offset 0x088 $reg
  set_property size           [expr {8*8}]   $reg
  set regparam [::ipx::add_register_parameter -quiet {ASSOCIATED_BUSIF} $reg] 
  set_property value m_axi_gmem2 $regparam 

set reg      [::ipx::add_register -quiet "scalar_flag" $addr_block]
