...
use crate::api::{
    ...
    cuMemcpyDtoH_v2, cuMemcpyHtoD_v2, cuMemcpyPeer, cuMemsetD32Async, CUdeviceptr, CUstream, cuMemFreeAsync,
};
...
impl<T> DeviceMemory<T> {
    ...
    pub fn read_from_async_cu(&self, src: &[T], size: usize, stream: CUstream) -> CudaResult<()> {
        let n = std::cmp::min(self.size, size);
    ...
    pub fn memset_async(&self, value: u32, size: usize, stream: &CudaStream) -> CudaResult<()> {
        let n = std::cmp::min(self.size, size);

        let size_in_u32 = std::mem::size_of::<T>() * n / 4;

        let result =
            unsafe { cuMemsetD32Async(self.inner, value, size_in_u32 as u64, stream.get_inner()) };
    ...
    pub fn memcpy_from_to_raw_async(
        ...
        let result = unsafe { cuMemcpyDtoDAsync_v2(dst, src, size, stream.get_inner()) };
