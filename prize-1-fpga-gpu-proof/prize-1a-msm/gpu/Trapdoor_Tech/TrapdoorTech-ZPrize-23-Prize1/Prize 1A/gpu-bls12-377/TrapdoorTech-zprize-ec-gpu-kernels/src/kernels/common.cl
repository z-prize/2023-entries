...
#if defined(__WinterPark__) || defined(__BeaverCreek__) ||                     \
    defined(__Turks__) || defined(__Caicos__) || defined(__Tahiti__) ||        \
    defined(__Pitcairn__) || defined(__Capeverde__) || defined(__Cayman__) ||  \
    defined(__Barts__) || defined(__Cypress__) || defined(__Juniper__) ||      \
    defined(__Redwood__) || defined(__Cedar__) || defined(__ATI_RV770__) ||    \
    defined(__ATI_RV730__) || defined(__ATI_RV710__) ||                        \
    defined(__Loveland__) || defined(__GPU__) || defined(__Hawaii__)
#define AMD
#endif
...
DEVICE inline void chain_init(chain_t *c) { c->_position = 0; }

DEVICE inline uint32_t chain_add(chain_t *ch, uint32_t a, uint32_t b) {
  uint32_t r;
...
DEVICE inline uint32_t chain_madlo(chain_t *ch, uint32_t a, uint32_t b,
                                   uint32_t c) {
  uint32_t r;
...
DEVICE inline uint32_t chain_madhi(chain_t *ch, uint32_t a, uint32_t b,
                                   uint32_t c) {
  uint32_t r;
