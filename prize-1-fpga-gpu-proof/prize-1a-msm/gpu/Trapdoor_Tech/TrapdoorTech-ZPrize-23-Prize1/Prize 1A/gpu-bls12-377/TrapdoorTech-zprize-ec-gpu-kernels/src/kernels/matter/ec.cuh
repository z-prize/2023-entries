...
struct ec
{
  ...
  struct point_ed_xytz;
  struct point_ed_affine;
  ...
  static __host__ __device__ __forceinline__ point_jacobian dbl_2009_l(const point_jacobian &point, const FD &fd)
  {
  ...
  template <bool CHECK_ZERO = true>
  static __host__ __device__ __forceinline__ point_jacobian dbl(const point_jacobian &point, const FD &fd)
  {
  ...
  template <bool CHECK_ZERO = true>
  static __host__ __device__ __forceinline__ point_xyzz dbl_2008_s_1(const point_xyzz &point, const FD &fd)
  {
    const storage X1 = point.x;     // < 2
    const storage Y1 = point.y;     // < 2
  ...
  template <bool CHECK_ZERO = true>
  static __host__ __device__ __forceinline__ point_xyzz dbl(const point_xyzz &point, const FD &fd)
  {
...
#pragma unroll
  ...
  template <bool CHECK_ZERO = true>
  static __host__ __device__ __forceinline__ point_xyzz mul(const unsigned scalar, const point_xyzz &point, const FD &fd)
  {
    if (CHECK_ZERO)
    {
      if (unlikely(fd.is_zero(point.z)))
...
#pragma unroll
  ...
  struct point_ed_xytz
  {
    ...
    static constexpr __host__ __device__ __forceinline__ point_ed_xytz point_at_infinity(const FD &fd) { return {{0}, fd.get_one(), {0}, fd.get_one()}; };
    static __host__ __device__ __forceinline__ point_ed_xytz to_ed_xytz(const point_ed_affine &point, const FD &fd)
    {
      const storage x = point.x;
      const storage y = point.y;
      const storage t = point.t;
      const storage z = fd.get_one();

      return {x, y, t, z};
    }
  };

  static __host__ __device__ __forceinline__ point_ed_xytz point_ed_xytz_add(const point_ed_xytz &p1, const point_ed_xytz &p2, const FD &fd)
  {
    // all x, y, t, z < 2M
    const storage y1_sub_x1 = fd.template sub<2>(p1.y, p1.x); // < 2
    const storage y2_sub_x2 = fd.template sub<2>(p2.y, p2.x); // < 2
    const storage y1_add_x1 = fd.template add<2>(p1.y, p1.x); // < 2
    const storage y2_add_x2 = fd.template add<2>(p2.y, p2.x); // < 2

    storage A = fd.template mul<0>(y1_sub_x1, y2_add_x2); // < 2
    storage B = fd.template mul<0>(y1_add_x1, y2_sub_x2); // < 2

    storage F = fd.template sub<2>(B, A); // < 2

    storage C, D, E, G, H;
    if (fd.is_zero(fd.reduce(F)))
    {
      // Use double here
      A = fd.template sqr<0>(p1.x); // < 2
      B = fd.template sqr<0>(p1.y); // < 2
      C = fd.template sqr<0>(p1.z); // < 2
      C = fd.template dbl<2>(C);    // < 2
      D = fd.template neg<2>(A);    // < 2

      E = fd.template sqr<0>(y1_add_x1); // < 2
      E = fd.template sub<2>(E, A);      // < 2
      E = fd.template sub<2>(E, B);      // < 2

      G = fd.template add<2>(D, B); // < 2
      F = fd.template sub<2>(G, C); // < 2
      H = fd.template sub<2>(D, B); // < 2
    }
    else
    {
      C = fd.template mul<0>(p1.z, p2.t); // < 2
      C = fd.template dbl<2>(C);          // < 2
      D = fd.template mul<0>(p1.t, p2.z); // < 2
      D = fd.template dbl<2>(D);          // < 2

      E = fd.template add<2>(D, C); // < 2
      H = fd.template sub<2>(D, C); // < 2
      G = fd.template add<2>(B, A); // < 2
    }

    C = fd.template mul<1>(E, F); // < 2
    D = fd.template mul<1>(G, H); // < 2
    E = fd.template mul<1>(E, H); // < 2
    F = fd.template mul<1>(F, G); // < 2
    return {C, D, E, F};
  }
  ...
  static __host__ __device__ __forceinline__ point_ed_xytz point_ed_add_mix(const point_ed_xytz &p1, const point_ed_affine &p2, const FD &fd)
  {
    // all x, y, t, z < 2M, affine x, y < 1M
    const storage y1_sub_x1 = fd.template sub<2>(p1.y, p1.x); // < 2
    const storage y2_sub_x2 = fd.template sub<1>(p2.y, p2.x); // < 1
    const storage y1_add_x1 = fd.template add<2>(p1.y, p1.x); // < 2
    const storage y2_add_x2 = fd.template add<0>(p2.y, p2.x); // < 2
    ...
    C = fd.template mul<0>(E, F); // < 2
    D = fd.template mul<0>(G, H); // < 2
    E = fd.template mul<0>(E, H); // < 2
    F = fd.template mul<0>(F, G); // < 2
    return {C, D, E, F};
  }
};
