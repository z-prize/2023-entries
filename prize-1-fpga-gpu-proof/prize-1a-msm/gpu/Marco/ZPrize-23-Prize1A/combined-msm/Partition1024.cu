...
__global__ void partition1024Kernel(int batch, void* pagesPtr, void* sizesPtr, void* countersPtr, void* processedScalarsPtr, uint32_t points, int WINDOWS_NUM, int BITS_PER_WINDOW) {
...
//  printf("Pointers: %lx %lx %lx %lx\n", (uint64_t)pagesPtr, (uint64_t)sizesPtr, (uint64_t)countersPtr, (uint64_t)processedScalarsPtr);
  ...
  while(true) {
    if(window*points+points<=chunk) {
      __syncthreads();
      while(window < WINDOWS_NUM && window * points + points <= chunk) {
        window++;
