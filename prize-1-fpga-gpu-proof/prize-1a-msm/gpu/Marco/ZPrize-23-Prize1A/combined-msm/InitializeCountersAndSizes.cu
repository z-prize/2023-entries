...
__global__ void initializeCountersSizesAtomicsHistogramKernel(void* countersPtr, void* sizesPtr, void* atomicsPtr, void* histogramPtr, int WINDOWS_NUM, int BITS_PER_WINDOW) {
  ...
  for(uint32_t i=globalTID; i <= WINDOWS_NUM * 1024; i+=globalStride) {
    if(i < WINDOWS_NUM * 1024)
      sizes[i]=0;
      
    if(i==0)
      counters[i]=make_wide(WINDOWS_NUM * 1024, 0);
    else
...
__global__ void sizesPrefixSumKernel(void* pagesPtr, void* prefixSumSizesPtr, void* sizesPtr, void* countersPtr, void* atomicsPtr, int WINDOWS_NUM, int BITS_PER_WINDOW) {
