...
template <typename FD> struct ec {
  ...
  struct point_affine {
    ...
    char* toHexString(const void* data, char* hexdata, size_t size) {
          const unsigned char* bytes = (const unsigned char*)data;
          char* hexStr = hexdata;

          hexStr[size * 2] = '\0'; // 结尾添加空字符

          // 将每个字节转换为十六进制字符串
          for (size_t i = 0; i < size; ++i) {
              int value = bytes[i];
              hexStr[i * 2] = "0123456789abcdef"[value >> 4]; // 高位
              hexStr[i * 2 + 1] = "0123456789abcdef"[value & 0x0F]; // 低位
          }
      ...
      __host__ __device__ __forceinline__ void print(const char* prompt = "") {
        char strdata[1024];
        toHexString(this, strdata, sizeof(*this));
        printf("%s%s\n", prompt, strdata);
    }
  ...
  struct point_xyzz {
    ...
    __host__ __device__ __forceinline__ point_affine to_affine() {
          auto value = this->to_projective(*this, FD());
          const auto inverse = field::inverse(value.z);
          const auto local_x = field::mul(value.x, inverse);
          const auto local_y = field::mul(value.y, inverse);

          auto aff = point_affine {local_x, local_y};
          return aff;
      }
