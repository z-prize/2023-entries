...
class HostReduce {
  typedef typename Host::PointXYZZ2<Field>       PointXYZZ;
  typedef typename Host::AccumulatorXYZZ2<Field> AccumulatorXYZZ;

  public:
  uint32_t windows, windowBits, warps, warpsPerWindow, bucketsPerThread;
  bool is381;
  const BLS12384Param* blsparam;
  HostReduce(uint32_t initialWindows, uint32_t initialWindowBits, uint32_t initialWarps, bool _is381) {
    ...
    if (is381)
        blsparam = &Host::param_381;
    else
        blsparam = &Host::param_377;
  }

    void reduceWindow(AccumulatorXYZZ& result, uint32_t* warpResults) {
        AccumulatorXYZZ sum(blsparam), sos(blsparam), interior(blsparam), scaled(blsparam);
        PointXYZZ       point(blsparam);
    ...
    void reduce(uint64_t *msmResult, uint32_t* warpResults) {
        AccumulatorXYZZ result(blsparam), windowResult(blsparam);
