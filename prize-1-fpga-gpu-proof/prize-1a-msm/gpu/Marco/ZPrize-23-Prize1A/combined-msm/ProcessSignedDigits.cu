...
__device__ __forceinline__ void slice_packed(uint32_t* sliced, const uint32_t* packed, int WINDOWS_NUM, int BITS_PER_WINDOW) {
//#if !USE_381
#if 0
  ...
  sliced[11]=0;
    ...
    for (int i = 0; i < WINDOWS_NUM; ++i) {
        uint32_t window = 0;
        int bitsRemaining = min(BITS_PER_WINDOW, total_bits-BITS_PER_WINDOW*i);
        int last_bits_copyed = 0;
        while (bitsRemaining > 0) {
            assert(srcIndex < 8);
            int bitsToCopy = min(bitsRemaining, 32 - bitOffset);
            window |= ((packed[srcIndex] >> bitOffset) & ((1u << bitsToCopy) - 1)) << last_bits_copyed;
            bitsRemaining -= bitsToCopy;
            bitOffset += bitsToCopy;
            last_bits_copyed += bitsToCopy;
...
__global__ void processSignedDigitsKernel(uint32_t batch, void* processedScalarData, void* scalarData, uint32_t points, int WINDOWS_NUM, int BITS_PER_WINDOW) {
  uint32_t  warpThread=threadIdx.x & 0x1F, warp=threadIdx.x>>5;  
  uint32_t  current=0, distributed=0, mask=(1<<threadIdx.x)>>1, estimate;
  uint32_t  packed[8], nTerm[8], sliced[12 + 1];
...
//        localN[6]=0x9a2ca556;
//        localN[7]=0x12ab655e;
//        assert(!sub(nTerm, packed, localN));
//
//    }
...
//  }
    ...
    slice_packed(sliced, packed, WINDOWS_NUM, BITS_PER_WINDOW);
