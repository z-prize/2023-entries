...
fn main() {
    ...
    MerkleTreeCircuit::init(&pk, &pp.powers_of_g);
    ...
    for i in 0..REPEAT {
        let (proof, pi) = {
            // Creating Prover and call to gadget() removed, as
            // we are doing witness generation on device.

            let mut pi = PublicInputs::new();
            pi.add_input(
                pi_pos[0],
                &(-real_circuits[i].merkle_tree.non_leaf_nodes[0]),
            )
            .unwrap();

            real_circuits[i]
                .gen_proof_accel::<KZG10<Bls12_381>>(
                    b"Merkle tree",
                    &pp.powers_of_g,
                    pi,
                )
