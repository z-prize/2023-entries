...
KERNEL void initialize_buckets_kernel(point_xyzz *buckets, const unsigned count)
{
  const unsigned gid = blockIdx.x * blockDim.x + threadIdx.x;
...
#define MIN_BLOCKS 16
KERNEL void compute_bucket_indexes_kernel(const fd_q::storage *__restrict__ scalars, const unsigned source_windows_count, const unsigned window_bits,
...
#define MAX_THREADS 32
#define MIN_BLOCKS 8
KERNEL void aggregate_buckets_kernel(bool IS_FIRST, const unsigned *__restrict__ base_indexes, const unsigned *__restrict__ bucket_run_offsets,
...
KERNEL void split_windows_kernel_generic(const unsigned source_window_bits_count, const unsigned source_windows_count,
...
KERNEL void split_windows_kernel_specialized(unsigned source_window_bits_count, unsigned source_windows_count,
                                      const point_xyzz *__restrict__ source_buckets, point_xyzz *__restrict__ target_buckets, const unsigned count)
{
  split_windows_kernel_inner(source_window_bits_count, source_windows_count, source_buckets, target_buckets, count);
}
...
#define MAX_THREADS 32
#define MIN_BLOCKS 12
KERNEL void reduce_buckets_kernel(point_xyzz *buckets, const unsigned count)
{
  const field f = field();
  const unsigned gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid >= count)
...
KERNEL void test_kernel(point_xyzz *__restrict__ buckets, point_jacobian *__restrict__ target, const unsigned count)
{
