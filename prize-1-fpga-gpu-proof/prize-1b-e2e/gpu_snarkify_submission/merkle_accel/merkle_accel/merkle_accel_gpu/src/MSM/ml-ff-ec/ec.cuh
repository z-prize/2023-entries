...
template <typename FD> struct ml_ec {
  typedef FD field;
  ...
  struct point_affine {
    ...
    static  __device__ __forceinline__ point_affine to_montgomery(const point_affine &point, const FD &fd) {
      const storage x = fd.to_montgomery(point.x);
      const storage y = fd.to_montgomery(point.y);
      return {x, y};
    }

    static  __device__ __forceinline__ point_affine from_montgomery(const point_affine &point, const FD &fd) {
      const storage x = fd.from_montgomery(point.x);
      const storage y = fd.from_montgomery(point.y);
      return {x, y};
    }

    static  __device__ __forceinline__ point_affine neg(const point_affine &point, const FD &fd) { return {point.x, fd.neg(point.y)}; }

    static  __device__ __forceinline__ point_projective to_projective(const point_affine &point, const FD &fd) {
      return {point.x, point.y, fd.get_one()};
    }

    static  __device__ __forceinline__ point_jacobian to_jacobian(const point_affine &point, const FD &fd) { return {point.x, point.y, fd.get_one()}; }

    static  __device__ __forceinline__ point_xyzz to_xyzz(const point_affine &point, const FD &fd) {
      return {point.x, point.y, fd.get_one(), fd.get_one()};
    }

    //  y^2=x^3+b
    static  __device__ __forceinline__ bool is_on_curve(const point_affine &point, const FD &fd) {
  ...
  struct point_projective {
    ...
    static constexpr  __device__ __forceinline__ point_projective point_at_infinity(const FD &fd) { return {{0}, fd.get_one(), {0}}; };

    static  __device__ __forceinline__ point_projective to_montgomery(const point_projective &point, const FD &fd) {
    ...
    static  __device__ __forceinline__ point_projective from_montgomery(const point_projective &point, const FD &fd) {
    ...
    static  __device__ __forceinline__ point_projective neg(const point_projective &point, const FD &fd) {
      return {point.x, fd.template neg<2>(point.y), point.z};
    }

    static  __device__ __forceinline__ bool eq(const point_projective &p1, const point_projective &p2, const FD &fd) {
    ...
    static  __device__ __forceinline__ point_jacobian to_jacobian(const point_projective &point, const FD &fd) {
    ...
    static  __device__ __forceinline__ point_xyzz to_xyzz(const point_projective &point, const FD &fd) {
      const storage z = point.z;
    ...
    static  __device__ __forceinline__ bool is_on_curve(const point_projective &point, const FD &fd) {
      const storage x = point.x;
      const storage y = point.y;
  ...
  struct point_jacobian {
    ...
    static constexpr  __device__ __forceinline__ point_jacobian point_at_infinity(const FD &fd) { return {{0}, fd.get_one(), {0}}; };

    static  __device__ __forceinline__ point_jacobian to_montgomery(const point_jacobian &point, const FD &fd) {
      const storage x = fd.to_montgomery<0>(point.x);
      const storage y = fd.to_montgomery<0>(point.y);
      const storage z = fd.to_montgomery<0>(point.z);
      return {x, y, z};
    ...
    static  __device__ __forceinline__ point_jacobian from_montgomery(const point_jacobian &point, const FD &fd) {
      const storage x = fd.from_montgomery<0>(point.x);
      const storage y = fd.from_montgomery<0>(point.y);
      const storage z = fd.from_montgomery<0>(point.z);
      return {x, y, z};
    ...
    static  __device__ __forceinline__ point_jacobian neg(const point_jacobian &point, const FD &fd) {
      return {point.x, fd.template neg<2>(point.y), point.z};
    }

    static  __device__ __forceinline__ bool eq(const point_jacobian &p1, const point_jacobian &p2, const FD &fd) {
      const storage z1 = fd.reduce(p1.z);
      const storage z2 = fd.reduce(p2.z);
      if (fd.is_zero(z1) != fd.is_zero(z2))
        return false;
    ...
    static  __device__ __forceinline__ point_projective to_projective(const point_jacobian &point, const FD &fd) {
      const storage x = fd.template mul<0>(point.x, point.z);
      const storage y = point.y;
    ...
    static bool  __device__ __forceinline__ is_on_curve(const point_jacobian &point, const FD &fd) {
      const storage x = point.x;
      const storage y = point.y;
      const storage z = fd.reduce(point.z);
      if (fd.is_zero(z))
  ...
  struct point_xyzz {
    ...
    static constexpr  __device__ __forceinline__ point_xyzz point_at_infinity(const FD &fd) { return {{0}, fd.get_one(), {0}, {0}}; };

    static  __device__ __forceinline__ point_xyzz to_montgomery(const point_xyzz &point, const FD &fd) {
      const storage x = fd.to_montgomery<0>(point.x);
      const storage y = fd.to_montgomery<0>(point.y);
    ...
    static  __device__ __forceinline__ point_xyzz from_montgomery(const point_xyzz &point, const FD &fd) {
      const storage x = fd.from_montgomery<0>(point.x);
      const storage y = fd.from_montgomery<0>(point.y);
    ...
    static  __device__ __forceinline__ point_xyzz neg(const point_xyzz &point, const FD &fd) {
      return {point.x, fd.template neg<2>(point.y), point.zz, point.zzz};
    }

    static  __device__ __forceinline__ bool eq(const point_xyzz &p1, const point_xyzz &p2, const FD &fd) {
    ...
    static  __device__ __forceinline__ point_projective to_projective(const point_xyzz &point, const FD &fd) {
    ...
    static  __device__ __forceinline__ point_jacobian to_jacobian(const point_xyzz &point, const FD &fd) {
    ...
    // y=Y/Z^3
    // y^2=x^3+b => Y^2/Z^6=X^3/Z^6+b => Y^2 = X^3 + b*Z^6
    static bool  __device__ __forceinline__ is_on_curve(const point_xyzz &point, const FD &fd) {
      const storage x = point.x;
      const storage y = point.y;
  ...
  template <bool CHECK_ZERO = true> static  __device__ __forceinline__ point_jacobian dbl_2009_l(const point_jacobian &point, const FD &fd) {
  ...
  template <bool CHECK_ZERO = true> static  __device__ __forceinline__ point_jacobian dbl(const point_jacobian &point, const FD &fd) {
    return dbl_2009_l<CHECK_ZERO>(point, fd);
  }

  // http://www.hyperelliptic.org/EFD/g1p/auto-shortw-jacobian-0.html#addition-add-2007-bl
  template <bool CHECK_ZERO = true, bool CHECK_DOUBLE = true>
  static  __device__ __forceinline__ point_jacobian add_2007_bl(const point_jacobian &p1, const point_jacobian &p2, const FD &fd) {
  ...
  template <bool CHECK_ZERO = true, bool CHECK_DOUBLE = true>
  static  __device__ __forceinline__ point_jacobian add(const point_jacobian &p1, const point_jacobian &p2, const FD &fd) {
  ...
  template <bool CHECK_ZERO = true, bool CHECK_DOUBLE = true>
  static  __device__ __forceinline__ point_jacobian add_madd_2008_q(const point_jacobian &p1, const point_affine &p2, const FD &fd) {
  ...
  template <bool CHECK_ZERO = true, bool CHECK_DOUBLE = true>
  static  __device__ __forceinline__ point_jacobian add(const point_jacobian &p1, const point_affine &p2, const FD &fd) {
  ...
  static  __device__ __forceinline__ point_xyzz mdbl_2008_s_1(const point_affine &point, const FD &fd) {
  ...
  static  __device__ __forceinline__ point_xyzz dbl(const point_affine &point, const FD &fd) { return mdbl_2008_s_1(point, fd); }

  // https://hyperelliptic.org/EFD/g1p/auto-shortw-xyzz.html#doubling-dbl-2008-s-1
  template <bool CHECK_ZERO = true> static  __device__ __forceinline__ point_xyzz dbl_2008_s_1(const point_xyzz &point, const FD &fd) {
    const storage X1 = point.x;     // < 2
    const storage Y1 = point.y;     // < 2
  ...
  template <bool CHECK_ZERO = true> static  __device__ __forceinline__ point_xyzz dbl(const point_xyzz &point, const FD &fd) {
    return dbl_2008_s_1<CHECK_ZERO>(point, fd);
  }

  // https://hyperelliptic.org/EFD/g1p/auto-shortw-xyzz.html#addition-add-2008-s
  template <bool CHECK_ZERO = true, bool CHECK_DOUBLE = true>
  static  __device__ __forceinline__ point_xyzz add_2008_s(const point_xyzz &p1, const point_xyzz &p2, const FD &fd) {
    const storage X1 = p1.x;     // < 2
    const storage Y1 = p1.y;     // < 2
  ...
  template <bool CHECK_ZERO = true, bool CHECK_DOUBLE = true>
  static  __device__ __forceinline__ point_xyzz add(const point_xyzz &p1, const point_xyzz &p2, const FD &fd) {
  ...
  template <bool CHECK_ZERO = true, bool CHECK_DOUBLE = true>
  static  __device__ __forceinline__ point_xyzz add_madd_2008_s(const point_xyzz &p1, const point_affine &p2, const FD &fd) {
    const storage X1 = p1.x;     // < 2
    const storage Y1 = p1.y;     // < 2
    const storage ZZ1 = p1.zz;   // < 2
    const storage ZZZ1 = p1.zzz; // < 2
  ...
  template <bool CHECK_ZERO = true, bool CHECK_DOUBLE = true>
  static  __device__ __forceinline__ point_xyzz add(const point_xyzz &p1, const point_affine &p2, const FD &fd) {
    return add_madd_2008_s<CHECK_ZERO, CHECK_DOUBLE>(p1, p2, fd);
  }

  static  __device__ __forceinline__ point_projective add(const point_projective &p1, const point_projective &p2, const FD &fd) {
    const storage X1 = p1.x;                                                 //                   < 2
    const storage Y1 = p1.y;                                                 //                   < 2
    const storage Z1 = p1.z;                                                 //                   < 2
    const storage X2 = p2.x;                                                 //                   < 2
  ...
  static  __device__ __forceinline__ point_projective add(const point_projective &p1, const point_affine &p2, const FD &fd) {
    const storage X1 = p1.x;
    const storage Y1 = p1.y;
    const storage Z1 = p1.z;
    const storage X2 = p2.x;
  ...
  // https://eprint.iacr.org/2015/1060.pdf
  static  __device__ __forceinline__ point_projective dbl(const point_projective &point, const FD &fd) {
    const storage X = point.x;
    const storage Y = point.y;
    const storage Z = point.z;
  ...
  static  __device__ __forceinline__ point_projective mul(const unsigned scalar, const point_projective &point, const FD &fd) {
...
#pragma unroll
  ...
  static  __device__ __forceinline__ point_jacobian mul(const unsigned scalar, const point_jacobian &point, const FD &fd) {
    if (CHECK_ZERO) {
...
#pragma unroll
  ...
  template <bool CHECK_ZERO = true> static  __device__ __forceinline__ point_xyzz mul(const unsigned scalar, const point_xyzz &point, const FD &fd) {
    if (CHECK_ZERO) {
      if (unlikely(fd.is_zero(point.z)))
...
#pragma unroll
  ...
  static  __device__ __forceinline__ point_projective mul(const typename FD_SCALAR::storage &scalar, const point_projective &point, const FD &fd) {
...
#pragma unroll
  ...
  static  __device__ __forceinline__ point_jacobian mul(const typename FD_SCALAR::storage &scalar, const point_jacobian &point, const FD &fd) {
    if (CHECK_ZERO) {
      if (unlikely(fd.is_zero(point.z)))
        return point_jacobian::point_at_infinity(fd);
    }
...
#pragma unroll
  ...
  template <class FD_SCALAR, bool CHECK_ZERO = true>
  static  __device__ __forceinline__ point_xyzz mul(const typename FD_SCALAR::storage &scalar, const point_xyzz &point, const FD &fd) {
    if (CHECK_ZERO) {
...
#pragma unroll
  ...
  static  __device__ __forceinline__ point_projective sub(const point_projective &p1, const point_affine &p2, const FD &fd) {
    return add(p1, point_affine::neg(p2, fd), fd);
  }

  static  __device__ __forceinline__ point_projective sub(const point_projective &p1, const point_projective &p2, const FD &fd) {
    return add(p1, point_projective::neg(p2, fd), fd);
  }

  template <bool CHECK_ZERO = true, bool CHECK_DOUBLE = true>
  static  __device__ __forceinline__ point_jacobian sub(const point_jacobian &p1, const point_affine &p2, const FD &fd) {
    return add<CHECK_ZERO, CHECK_DOUBLE>(p1, point_affine::neg(p2, fd), fd);
  }

  template <bool CHECK_ZERO = true, bool CHECK_DOUBLE = true>
  static  __device__ __forceinline__ point_jacobian sub(const point_jacobian &p1, const point_jacobian &p2, const FD &fd) {
    return add<CHECK_ZERO, CHECK_DOUBLE>(p1, point_jacobian::neg(p2, fd), fd);
  }

  template <bool CHECK_ZERO = true, bool CHECK_DOUBLE = true>
  static  __device__ __forceinline__ point_xyzz sub(const point_xyzz &p1, const point_affine &p2, const FD &fd) {
    return add<CHECK_ZERO, CHECK_DOUBLE>(p1, point_affine::neg(p2, fd), fd);
  }

  template <bool CHECK_ZERO = true, bool CHECK_DOUBLE = true>
  static  __device__ __forceinline__ point_xyzz sub(const point_xyzz &p1, const point_xyzz &p2, const FD &fd) {
