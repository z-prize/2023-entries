...
template <class FF_CONFIG, const uint32_t &INV> struct ff_dispatch_st {
  ...
  // return modulus
  template <unsigned MULTIPLIER = 1> static constexpr DEVICE_INLINE storage get_modulus() {
    switch (MULTIPLIER) {
    case 1:
  ...
  template <unsigned MULTIPLIER = 1> static constexpr DEVICE_INLINE storage_wide get_modulus_squared() {
    switch (MULTIPLIER) {
    case 1:
  ...
  // return r^2
  static constexpr DEVICE_INLINE storage get_r2() { return CONFIG::r2; }

  // return one in montgomery form
  static constexpr DEVICE_INLINE storage get_one() { return CONFIG::one; }
...
#pragma unroll
  ...
  template <bool SUBTRACT, bool CARRY_OUT, typename T> static constexpr DEVICE_INLINE uint32_t add_sub_limbs(const T &xs, const T &ys, T &rs) {
  ...
  template <bool CARRY_OUT, typename T> static constexpr DEVICE_INLINE uint32_t add_limbs(const T &xs, const T &ys, T &rs) {
    return add_sub_limbs<false, CARRY_OUT>(xs, ys, rs);
  }

  template <bool CARRY_OUT, typename T> static constexpr DEVICE_INLINE uint32_t sub_limbs(const T &xs, const T &ys, T &rs) {
...
#pragma unroll
  ...
  template <unsigned REDUCTION_SIZE = 1> static constexpr DEVICE_INLINE storage reduce(const storage &xs) {
    if (REDUCTION_SIZE == 0)
      return xs;
  ...
  template <unsigned REDUCTION_SIZE = 1> static constexpr DEVICE_INLINE storage_wide reduce_wide(const storage_wide &xs) {
    if (REDUCTION_SIZE == 0)
      return xs;
  ...
  // return xs + ys with field operands
  template <unsigned REDUCTION_SIZE = 1> static constexpr DEVICE_INLINE storage add(const storage &xs, const storage &ys) {
    storage rs = {};
    add_limbs<false>(xs, ys, rs);
    return reduce<REDUCTION_SIZE>(rs);
  }

  template <unsigned REDUCTION_SIZE = 1> static constexpr DEVICE_INLINE storage_wide add_wide(const storage_wide &xs, const storage_wide &ys) {
    storage_wide rs = {};
    add_limbs<false>(xs, ys, rs);
  ...
  // return xs - ys with field operands
  template <unsigned REDUCTION_SIZE = 1> static DEVICE_INLINE storage sub(const storage &xs, const storage &ys) {
    storage rs = {};
    if (REDUCTION_SIZE == 0) {
  ...
  template <unsigned REDUCTION_SIZE = 1> static DEVICE_INLINE storage_wide sub_wide(const storage_wide &xs, const storage_wide &ys) {
    storage_wide rs = {};
    if (REDUCTION_SIZE == 0) {
      sub_limbs<false>(xs, ys, rs);
    } else {
...
#pragma unroll
  ...
  // Host path uses CIOS.
  template <unsigned REDUCTION_SIZE = 1> static constexpr DEVICE_INLINE storage mul(const storage &xs, const storage &ys) {
    return mul_device<REDUCTION_SIZE>(xs, ys);
  }

  // convert field to montgomery form
  template <unsigned REDUCTION_SIZE = 1> static constexpr DEVICE_INLINE storage to_montgomery(const storage &xs) {
    constexpr storage r2 = CONFIG::r2;
    return mul<REDUCTION_SIZE>(xs, r2);
  }

  // convert field from montgomery form
  template <unsigned REDUCTION_SIZE = 1> static constexpr DEVICE_INLINE storage from_montgomery(const storage &xs) { return mul<REDUCTION_SIZE>(xs, {1}); }

  // return xs^2 with field operands
  template <unsigned REDUCTION_SIZE = 1> static constexpr DEVICE_INLINE storage sqr(const storage &xs) {
...
#pragma unroll
  ...
  template <unsigned REDUCTION_SIZE = 1> static constexpr DEVICE_INLINE storage dbl(const storage &xs) {
...
#pragma unroll
  ...
  template <unsigned REDUCTION_SIZE = 1> static constexpr DEVICE_INLINE storage div2(const storage &xs) {
    return div2_device<REDUCTION_SIZE>(xs);
  }

  // return -xs with field operand
  template <unsigned MODULUS_SIZE = 1> static constexpr DEVICE_INLINE storage neg(const storage &xs) {
  ...
  static constexpr DEVICE_INLINE storage mul(const unsigned scalar, const storage &xs) {
    storage rs = {};
    storage temp = xs;
    unsigned l = scalar;
    bool is_zero = true;
#pragma unroll
  ...
  static constexpr DEVICE_INLINE bool is_odd(const storage &xs) { return xs.limbs[0] & 1; }

  static constexpr DEVICE_INLINE bool is_even(const storage &xs) { return ~xs.limbs[0] & 1; }
