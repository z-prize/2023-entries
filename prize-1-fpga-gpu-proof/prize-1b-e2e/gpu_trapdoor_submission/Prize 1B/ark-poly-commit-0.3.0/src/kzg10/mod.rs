...
use ec_gpu_common::{
    gpu_msm, CUdeviceptr, G1Affine, MSMContext, MultiexpKernel,
};
    ...
    for<'a, 'b> &'a P: Div<&'b P, Output = P>,
{
        ...
        G: AffineCurve,
    {
        ...
        msm_kern.multi_scalar_mult_init(bases)
    }
        ...
        G: AffineCurve,
    {
        ...
        let (num_leading_zeros, mut plain_coeffs) =
        ...
        if plain_coeffs.len() < 1 << 4 || msm_context.is_none() {
            ...
            let msm_ctx = msm_context.unwrap();

            Self::powers_of_2(1 << msm_ctx.get_log_count(), &mut plain_coeffs);
            let scalars =
                unsafe { transmute::<_, &Vec<BigInteger256>>(&plain_coeffs) };
            panic!("------");

            let commitment_gpu = gpu_msm(scalars, None, msm_ctx, true);
            let commitment_gpu: &<E as PairingEngine>::G1Projective = unsafe {
                transmute::<_, &<E as PairingEngine>::G1Projective>(
                    &commitment_gpu,
                )
            };
    ...
    pub fn commit_gpu_scalars<'a, 'b, G>(
        powers: &Powers<E>,
        polynomials_gpu: CUdeviceptr,
        n: usize,
        hiding_bound: Option<usize>,
        rng: Option<&mut dyn RngCore>,
        msm_context: Option<&mut MSMContext<'a, 'b, G>>,
        ...
        G: AffineCurve,
    {
        ...
        let plain_coeffs = polynomials_gpu;
        let msm_time =
            start_timer!(|| "MSM to compute commitment to plaintext poly");

        let msm_ctx = msm_context.unwrap();

        let commitment = gpu_msm(&[], Some((plain_coeffs, n)), msm_ctx, false);
        let commitment: &<E as PairingEngine>::G1Projective = unsafe {
            transmute::<_, &<E as PairingEngine>::G1Projective>(&commitment)
        };

        end_timer!(msm_time);

        let randomness = Randomness::<E::Fr, P>::empty();

        end_timer!(commit_time);
        Ok((Commitment((*commitment).into()), randomness))
    }

    fn poly_div(p: &P, divisor: &P) -> Result<P, Error> {
        let witness_polynomial = p / &divisor;

        let ret = witness_polynomial.clone();
        Ok(ret)
    }
    ...
    pub fn compute_witness_polynomial(
        ...
        // let witness_polynomial = p / &divisor;
        let witness_polynomial = Self::poly_div(p, &divisor).unwrap();
    ...
    pub(crate) fn compute_witness_proof_gpu<'a, 'b, G: AffineCurve>(
        powers: &Powers<E>,
        coeffs: CUdeviceptr,
        n: usize,
        msm_context: Option<&mut MSMContext<'a, 'b, G>>,
        ...
        let commit =
            gpu_msm(&[], Some((coeffs, n)), msm_context.unwrap(), false);
        let commit: &<E as PairingEngine>::G1Projective = unsafe {
            transmute::<_, &<E as PairingEngine>::G1Projective>(&commit)
        };
        Ok(Proof {
            w: commit.into_affine(),
            random_v: None,
        })
    }
        ...
        G: AffineCurve,
    {
        ...
        let scalars =
            unsafe { transmute::<_, &Vec<BigInteger256>>(&witness_coeffs) };

        let commitment_gpu =
            gpu_msm(&scalars, None, msm_context.unwrap(), true);
        let w = unsafe {
            transmute::<_, &<E as PairingEngine>::G1Projective>(&commitment_gpu)
        };
        let mut w = w.clone();

        // let w_cpu = VariableBaseMSM::multi_scalar_mul(
        //     &powers.powers_of_g,
        //     &witness_coeffs,
        // );
