...
impl<E: PairingEngine, P: UVPolynomial<E::Fr>> SonicKZG10<E, P> {
    fn accumulate_elems_individual_opening_challenges<'a>(
        ...
        // Iterates through all of the commitments and accumulates common
        // degree_bound elements in a BTreeMap
        for (labeled_comm, value) in commitments.into_iter().zip(values) {
            ...
            // Applying opening challenge and randomness (used in
            // batch_checking)
            let mut comm_with_challenge: E::G1Projective =
                comm.0.mul(curr_challenge);

            if let Some(randomizer) = randomizer {
                comm_with_challenge =
                    comm_with_challenge.mul(&randomizer.into_repr());
            }
        ...
        // Push expected results into list of elems. Power will be the negative
        // of the expected power
        let mut witness: E::G1Projective = proof.w.into_projective();
        let mut adjusted_witness =
            vk.g.mul(combined_values) - &proof.w.mul(point);
    ...
    fn check_elems(
        ...
        let is_one: bool =
            E::product_of_pairings(g1_g2_prepared.iter()).is_one();
    ...
    for<'a, 'b> &'a P: Div<&'b P, Output = P>,
{
    ...
    fn trim(
        ...
        ) = if let Some(enforced_degree_bounds) =
            enforced_degree_bounds.as_ref()
        {
            if enforced_degree_bounds.is_empty() {
                (None, None, None)
            } else {
                let highest_enforced_degree_bound =
                    *enforced_degree_bounds.last().unwrap();
                if highest_enforced_degree_bound > supported_degree {
                    return Err(Error::UnsupportedDegreeBound(
                        highest_enforced_degree_bound,
                    ));
                }

                let lowest_shift_degree =
                    max_degree - highest_enforced_degree_bound;
                ...
                let shifted_powers_of_g =
                    pp.powers_of_g[lowest_shift_degree..].to_vec();
                ...
                for degree_bound in enforced_degree_bounds {
                    ...
                    for i in 0..=(supported_hiding_bound + 1) {
                        // We have an additional degree in `powers_of_gamma_g`
                        // beyond `powers_of_g`.
                        if shift_degree + i < max_degree + 2 {
                            powers_for_degree_bound.push(
                                pp.powers_of_gamma_g[&(shift_degree + i)],
                            );
                        }
                    }
                    shifted_powers_of_gamma_g
                ...
                let degree_bounds_and_neg_powers_of_h = enforced_degree_bounds
                    ...
                    .map(|bound| {
                        (
                            *bound,
                            neg_powers_of_h[&(max_degree - *bound)].clone(),
                        )
                    })
                    .collect();
    ...
    fn get_gpu_context<'a, 'b, G>(
        msm_kern: &'a MultiexpKernel<'static, 'static, G>,
        ck: &'b Self::CommitterKey,
        coeffs_count: usize,
    ) -> MSMContext<'a, 'b, G>
    where
        'a: 'b,
        G: AffineCurve,
    {
        ...
        kzg10::KZG10::get_gpu_context(msm_kern, &powers, coeffs_count)
    }
    ...
    fn commit<'a, 'b, 'c, G>(
        ...
        msm_kern: Option<&mut MSMContext<'b, 'c, G>>,
    ) -> Result<
        (
    ...
    where
        ...
        'b: 'a,
    {
        if let Some(msm_kern) = msm_kern {
            return Self::commit_gpu(ck, polynomials, rng, msm_kern);
        } else {
            return Self::commit_cpu(ck, polynomials, rng);
        }
    }
        ...
        'b: 'a,
    {
        ...
        let mut labeled_comms: Vec<LabeledCommitment<Self::Commitment>> =
            Vec::new();
        ...
        for labeled_polynomial in polynomials {
            ...
            let powers = if let Some(degree_bound) = degree_bound {
                println!(
                    "00000-- {}",
                    ck.shifted_powers(degree_bound).unwrap().powers_of_g.len()
                );
                assert!(false);
            ...
            //     g.write(&mut buffer).unwrap();

            //     println!("0-label:{}", label.to_string());
            //     // println!("commit :{:?}", &buffer[0..10]);
            // }
    ...
    {
        let powers = ck.powers();
        kzg10::KZG10::compute_witness_proof_gpu(
            &powers,
            scalars,
            n,
            msm_context,
        ...
        'b: 'a,
    {
        ...
        let mut labeled_comms: Vec<LabeledCommitment<Self::Commitment>> =
            Vec::new();
        let mut randomness: Vec<Self::Randomness> = Vec::new();
        for (index, polynomial_gpu) in polynomials_gpu.iter().enumerate() {
            let degree_bound = None;

            let powers = ck.powers();
            let (comm, rand) = kzg10::KZG10::commit_gpu_scalars(
                &powers,
                *polynomial_gpu,
                n,
                None,
            ...
            labeled_comms.push(LabeledCommitment::new(
                label_names.get(index).unwrap().to_string(),
                comm,
                degree_bound,
            ));
            randomness.push(rand);
    ...
    fn open_individual_opening_challenges<'a, 'b, 'c, G>(
        ...
        labeled_polynomials: impl IntoIterator<
            Item = &'a LabeledPolynomial<E::Fr, P>,
        >,
        ...
        Self::Commitment: 'a,
    {
        ...
        let mut combined_comms: BTreeMap<Option<usize>, E::G1Projective> =
            BTreeMap::new();
        let mut combined_witness: E::G1Projective = E::G1Projective::zero();
        let mut combined_adjusted_witness: E::G1Projective =
            E::G1Projective::zero();
        ...
        Self::Commitment: 'a,
    {
        ...
        {
            let mut comms_to_combine: Vec<&'_ LabeledCommitment<_>> =
                Vec::new();
