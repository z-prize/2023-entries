...
use crate::{
    PCCommitterKey, PCRandomness, PCUniversalParams, PolynomialCommitment,
};
...
impl<G: AffineCurve, D: Digest, P: UVPolynomial<G::ScalarField>>
    InnerProductArgPC<G, D, P>
{
    /// `PROTOCOL_NAME` is used as a seed for the setup function.
    pub const PROTOCOL_NAME: &'static [u8] = b"PC-DL-2020";

    /// Create a Pedersen commitment to `scalars` using the commitment key
    /// `comm_key`. Optionally, randomize the commitment using
    /// `hiding_generator` and `randomizer`.
    fn cm_commit(
        comm_key: &[G],
        ...
        let mut comm =
            VariableBaseMSM::multi_scalar_mul(comm_key, &scalars_bigint);
    ...
    fn succinct_check<'a>(
        ...
        // `log_d` is ceil(log2 (d + 1)), which is the number of steps to
        // compute all of the challenges
        ...
        for (labeled_commitment, value) in labeled_commitments.zip(values) {
            ...
            combined_commitment_proj +=
                &labeled_commitment.commitment().comm.mul(cur_challenge);
            ...
            if let Some(degree_bound) = degree_bound {
                let shift =
                    point.pow([(vk.supported_degree() - degree_bound) as u64]);
                combined_v += &(cur_challenge * &value * &shift);
                combined_commitment_proj +=
                    &commitment.shifted_comm.unwrap().mul(cur_challenge);
            }
        ...
        if proof.hiding_comm.is_some() {
            ...
            combined_commitment_proj +=
                &(hiding_comm.mul(hiding_challenge) - &vk.s.mul(rand));
        ...
        for (l, r) in l_iter.zip(r_iter) {
            ...
            round_commitment_proj += &(l
                .mul(round_challenge.inverse().unwrap())
                + &r.mul(round_challenge));
        }

        let check_poly =
            SuccinctCheckPolynomial::<G::ScalarField>(round_challenges);
    ...
    fn shift_polynomial(ck: &CommitterKey<G>, p: &P, degree_bound: usize) -> P {
        if p.is_zero() {
            ...
            let mut shifted_polynomial_coeffs = vec![
                G::ScalarField::zero();
                ck.supported_degree()
                    - degree_bound
            ];
    ...
    fn combine_shifted_rand(
        ...
        if let Some(new_rand) = new_rand {
            ...
            return Some(
                combined_rand.map_or(coeff_new_rand, |r| r + &coeff_new_rand),
            );
        };
    ...
    fn combine_shifted_comm(
        ...
        if let Some(new_comm) = new_comm {
            ...
            return Some(
                combined_comm.map_or(coeff_new_comm, |c| c + &coeff_new_comm),
            );
        };
    ...
    fn sample_generators(num_generators: usize) -> Vec<G> {
        let generators: Vec<_> = ark_std::cfg_into_iter!(0..num_generators)
            .map(|i| {
                let i = i as u64;
                let mut hash =
                    D::digest(&to_bytes![&Self::PROTOCOL_NAME, i].unwrap());
                let mut g = G::from_random_bytes(&hash);
                let mut j = 0u64;
                while g.is_none() {
                    hash = D::digest(
                        &to_bytes![&Self::PROTOCOL_NAME, i, j].unwrap(),
                    );
...
impl<G, D, P> PolynomialCommitment<G::ScalarField, P>
    for InnerProductArgPC<G, D, P>
where
    G: AffineCurve,
    D: Digest,
    P: UVPolynomial<G::ScalarField, Point = G::ScalarField>,
{
    ...
    fn setup<R: RngCore>(
        ...
        let setup_time =
            start_timer!(|| format!("Sampling {} generators", max_degree + 3));
    ...
    fn trim(
        ...
        let trim_time = start_timer!(|| format!(
            "Trimming to supported degree of {}",
            supported_degree
        ));
    ...
    fn commit<'a, 'b, 'c, T>(
        ...
        polynomials: impl IntoIterator<
            Item = &'a LabeledPolynomial<G::ScalarField, P>,
        >,
        rng: Option<&mut dyn RngCore>,
        _msm_context: Option<&mut MSMContext<'b, 'c, T>>,
    ) -> Result<
        (
    ...
    fn commit_gpu<'a, 'b, 'c, T>(
        ck: &Self::CommitterKey,
        polynomials: impl IntoIterator<
            Item = &'a LabeledPolynomial<G::ScalarField, P>,
        >,
        rng: Option<&mut dyn RngCore>,
        msm_context: &mut MSMContext<'b, 'c, T>,
    ) -> Result<
        (
            Vec<LabeledCommitment<Self::Commitment>>,
            Vec<Self::Randomness>,
    ...
    fn compute_witness_proof_gpu<'a, 'b, 'c, T>(
        ck: &Self::CommitterKey,
        scalars: CUdeviceptr,
        n: usize,
        msm_context: Option<&mut MSMContext<'b, 'c, T>>,
    ...
    fn commit_gpu_scalars<'a, 'b, 'c, T>(
        ck: &Self::CommitterKey,
        polynomials_gpu: Vec<CUdeviceptr>,
        n: usize,
    ...
    fn open_individual_opening_challenges<'a, 'b, 'c, T>(
        ck: &Self::CommitterKey,
        labeled_polynomials: impl IntoIterator<
            Item = &'a LabeledPolynomial<G::ScalarField, P>,
        >,
        commitments: impl IntoIterator<
            Item = &'a LabeledCommitment<Self::Commitment>,
        >,
        point: &'a P::Point,
        opening_challenges: &dyn Fn(u64) -> G::ScalarField,
        ...
        'b: 'a,
    {
        ...
        let combine_time = start_timer!(|| {
            "Combining polynomials, randomness, and commitments."
        });
            ...
            polys_iter.zip(comms_iter.zip(rands_iter))
        {
            ...
            if let Some(degree_bound) = degree_bound {
                let shifted_polynomial =
                    Self::shift_polynomial(ck, polynomial, degree_bound);
        ...
        if has_hiding {
            ...
            hiding_polynomial -= &P::from_coefficients_slice(&[
                hiding_polynomial.evaluate(point),
            ]);
            ...
            combined_commitment_proj +=
                &(hiding_commitment.unwrap().mul(hiding_challenge)
                    - &ck.s.mul(combined_rand));
        ...
        // Pads the coefficients with zeroes to get the number of coeff to be
        // d+1
        ...
        let mut key_proj: Vec<G::Projective> =
            ck.comm_key.iter().map(|x| (*x).into()).collect();
        let mut key_proj = key_proj.as_mut_slice();

        let mut temp;

        // Key for MSM
        // We initialize this to capacity 0 initially because we want to use the
        // key slice first
        ...
        Self::Commitment: 'a,
    {
        let commitments: BTreeMap<_, _> =
            commitments.into_iter().map(|c| (c.label(), c)).collect();
        ...
        for ((_point_label, (point, labels)), p) in
            query_to_labels_map.into_iter().zip(proof)
        {
            let lc_time = start_timer!(|| format!(
                "Randomly combining {} commitments",
                labels.len()
            ));
            ...
            for label in labels.into_iter() {
                let commitment =
                    commitments.get(label).ok_or(Error::MissingPolynomial {
                        label: label.to_string(),
                    })?;

                let v_i = values.get(&(label.clone(), *point)).ok_or(
            ...
            let check_poly =
                P::from_coefficients_vec(check_poly.unwrap().compute_coeffs());
        ...
        P: 'a,
    {
        ...
        for lc in lc_s {
            ...
            for (coeff, label) in lc.iter().filter(|(_, l)| !l.is_one()) {
                ...
                let &(cur_poly, cur_rand, cur_comm) = label_poly_map
                    .get(label)
                ...
                if num_polys == 1 && cur_poly.degree_bound().is_some() {
                    ...
                    eprintln!(
                        "Degree bound when number of equations is non-zero"
                    );
                ...
                // Some(_) > None, always.
                hiding_bound =
                    core::cmp::max(hiding_bound, cur_poly.hiding_bound());
            ...
            let lc_poly = LabeledPolynomial::new(
                lc_label.clone(),
                poly,
                degree_bound,
                hiding_bound,
        ...
        let lc_commitments =
            Self::construct_labeled_commitments(&lc_info, &lc_commitments);
        ...
        Self::Commitment: 'a,
    {
        ...
        for lc in lc_s {
            ...
            for (coeff, label) in lc.iter() {
                if label.is_one() {
                    for (&(ref label, _), ref mut eval) in
                        evaluations.iter_mut()
                    {
                        if label == &lc_label {
                            **eval -= coeff;
                        }
                    ...
                    let &cur_comm = label_comm_map.get(label).ok_or(
                        Error::MissingPolynomial {
                            label: label.to_string(),
                        },
                    )?;
                    ...
                    if num_polys == 1 && cur_comm.degree_bound().is_some() {
                        ...
                        return Err(Self::Error::EquationHasDegreeBounds(
                            lc_label,
                        ));
                    }
