...
use ec_gpu_common::{CUdeviceptr, MSMContext, MultiexpKernel};
...
/// A multilinear polynomial commitment scheme that converts n-variate
/// multilinear polynomial into n quotient UV polynomial. This scheme is based
/// on hardness of the discrete logarithm in prime-order groups. Construction is
/// detailed in [[XZZPD19]][xzzpd19] and [[ZGKPP18]][zgkpp18]
///
...
/// element of a `QuerySet` is a pair of `(label, (point_label, point))`, where
/// `label` is the label of a polynomial in `p`, `point_label` is the label for
/// the point (e.g., "beta"), and  and `point` is the location that `p[label]`
/// is to be queried at.
...
/// a sender to commit to multiple polynomials and later provide a succinct
/// proof of evaluation for the corresponding commitments at a query set `Q`,
/// while enforcing per-polynomial degree bounds.
pub trait PolynomialCommitment<F: Field, P: Polynomial<F>>: Sized {
    ...
    /// The committer key for the scheme; used to commit to a polynomial and
    /// then open the commitment to produce an evaluation proof.
    type CommitterKey: PCCommitterKey;
    /// The verifier key for the scheme; used to check an evaluation proof.
    type VerifierKey: PCVerifierKey;
    /// The prepared verifier key for the scheme; used to check an evaluation
    /// proof.
    ...
    /// `degree` for the polynomial commitment scheme. `num_vars` specifies
    /// the number of variables for multivariate setup
    fn setup<R: RngCore>(
        max_degree: usize,
    ...
    fn get_gpu_context<'a, 'b, G>(
        _msm_kern: &'a MultiexpKernel<'static, 'static, G>,
        _ck: &'b Self::CommitterKey,
        _coeffs_count: usize,
    ) -> MSMContext<'a, 'b, G>
    where
        'a: 'b,
    ...
    /// `supported_degree` and for enforcing degree bounds in the range
    /// `1..=supported_degree`.
    fn trim(
    ...
    /// then the `i`-th commitment is hiding up to `polynomials.hiding_bound()`
    /// queries. `rng` should not be `None` if `polynomials[i].is_hiding()
    /// == true` for any `i`.
    ///
    ...
    fn commit<'a, 'b, 'c, G>(
        ...
        msm_context: Option<&mut MSMContext<'b, 'c, G>>,
    ) -> Result<
        (
    ...
    fn compute_witness_proof_gpu<'a, 'b, 'c, G>(
        ck: &Self::CommitterKey,
        scalars: CUdeviceptr,
        n: usize,
        msm_context: Option<&mut MSMContext<'b, 'c, G>>,
    ...
    /// On input a list of labeled polynomials and a query point, `open` outputs
    /// a proof of evaluation of the polynomials at the query point.
    fn open<'a, 'b, 'c, G>(
        ck: &Self::CommitterKey,
    ...
    /// On input a list of polynomials, linear combinations of those
    /// polynomials, and a query set, `open_combination` outputs a proof of
    /// evaluation of the combinations at the points in the query set.
    ...
    fn open_individual_opening_challenges<'a, 'b, 'c, G>(
        ck: &Self::CommitterKey,
        labeled_polynomials: impl IntoIterator<Item = &'a LabeledPolynomial<F, P>>,
        commitments: impl IntoIterator<
            Item = &'a LabeledCommitment<Self::Commitment>,
        ...
        Self::Commitment: 'a,
    {
        ...
        let mut result = true;
        for ((_point_label, (point, labels)), proof) in
            query_to_labels_map.into_iter().zip(proofs)
        {
        ...
        P: 'a,
    {
        let linear_combinations: Vec<_> =
            linear_combinations.into_iter().collect();
        let polynomials: Vec<_> = polynomials.into_iter().collect();
        let poly_query_set = lc_query_set_to_poly_query_set(
            linear_combinations.iter().copied(),
            query_set,
        );
        let poly_evals =
            evaluate_query_set(polynomials.iter().copied(), &poly_query_set);
        ...
        Self::Commitment: 'a,
    {
        ...
        let lc_s = BTreeMap::from_iter(
            linear_combinations.into_iter().map(|lc| (lc.label(), lc)),
        );
        ...
        for &(ref lc_label, (_, ref point)) in eqn_query_set {
            if let Some(lc) = lc_s.get(lc_label) {
                ...
                if claimed_rhs != actual_rhs {
                    eprintln!(
                        "Claimed evaluation of {} is incorrect",
                        lc.label()
                    );
                    return Ok(false);
                }
        ...
        // G: AffineCurve,
    {
        ...
        for (_point_label, (point, labels)) in query_to_labels_map.into_iter() {
            ...
            let mut query_comms: Vec<&'a LabeledCommitment<Self::Commitment>> =
                Vec::new();

            for label in labels {
                let (polynomial, rand, comm) = poly_rand_comm
                    .get(label)
            ...
            let proof = Self::open_individual_opening_challenges(
                ...
                query_rands,
                Some(rng),
                None::<&mut MSMContext<'_, '_, G1Affine>>,
            )?;
        ...
        if let Some(lc) = linear_combinations.get(lc_label) {
            for (_, poly_label) in lc.iter().filter(|(_, l)| !l.is_one()) {
                if let LCTerm::PolyLabel(l) = poly_label {
                    poly_query_set.insert((
                        l.into(),
                        (point_label.clone(), point.clone()),
                    ));
                }
            }
        }
        ...
        PC: PolynomialCommitment<F, P>,
    {
        ...
        for _ in 0..10 {
            ...
            for (i, label) in labels.iter().enumerate() {
                query_set
                    .insert((label.clone(), (format!("{}", i), point.clone())));
        ...
        PC: PolynomialCommitment<F, P>,
    {
        ...
        for _ in 0..num_iters {
            let supported_degree = supported_degree
                .unwrap_or(Uniform::from(1..=max_degree).sample(rng));
            assert!(
                max_degree >= supported_degree,
                "max_degree < supported_degree"
            );
            ...
            let num_points_in_query_set =
                Uniform::from(1..=max_num_queries).sample(rng);
            for i in 0..num_polynomials {
                let label = format!("Test{}", i);
                labels.push(label.clone());
                let degree = Uniform::from(1..=supported_degree).sample(rng);
                let degree_bound =
                    if let Some(degree_bounds) = &mut degree_bounds {
        ...
        PC: PolynomialCommitment<F, P>,
    {
        ...
        for _ in 0..num_iters {
            ...
            for i in 0..num_polynomials {
                ...
                labels.push(label.clone());
                let degree = Uniform::from(1..=supported_degree).sample(rng);
                let degree_bound = if let Some(degree_bounds) =
                    &mut degree_bounds
                {
                    if rng.gen() {
                        let range = Uniform::from(degree..=supported_degree);
