...
static MULTIEXP_BUCKET_SRC: &str =
    include_str!("./kernels/matter/multiexp_bucket.cu");
static MATTER_LAB_EC_SRC: &str = include_str!("./kernels/matter/ec.cuh");
static COMMON_CUH_SRC: &str = include_str!("./kernels/matter/common.cuh");

static CARRY_CHAIN_SRC: &str = include_str!("./kernels/matter/carry_chain.cuh");
static FF_CONFIG_H_SRC: &str = include_str!("./kernels/matter/ff_config.cuh");
static FF_DISPATCH_ST_SRC: &str =
    include_str!("./kernels/matter/ff_dispatch_st.cuh");
static FF_STORAGE: &str = include_str!("./kernels/matter/ff_storage.cuh");
static MEMORY_SRC: &str = include_str!("./kernels/matter/memory.cuh");
static PTX_SRC: &str = include_str!("./kernels/matter/ptx.cuh");

static HOST_MATH_SRC: &str = include_str!("./kernels/matter/host_math.cuh");
static ZPRIZE_1B_PERMUTATION: &str = include_str!("./kernels/zprize_1b/permutation.cu");
...
pub fn gen_all_source<F1: Field, F2: Field, L: Limb>() -> String {
    vec![
        ...
        ptx("G1", "Fr"),
        carry_chain("G1", "Fr"),
        ff_dispatch_st("G1", "Fr"),
        memory("G1", "Fr"),
        common_cuh("G1", "Fr"),
        matter_lab_ec("G1", "Fr"),
        // cub("G1", "Fr"),
        multiexp_bucket("G1", "Fr"),
        zprize_permutation("Fr"),

    ]
    .join("\n\n")
}

/// Generates the source for the elliptic curve and group operations, as defined
/// by `E`.
///
/// The code from the [`common()`] call needs to be included before this on is
/// used.
...
pub fn gen_ed_a_d<L: Limb>(field: &str) -> String {
    ...
    let a_def = if TypeId::of::<L>() == TypeId::of::<Limb32>() {
        ...
        let limbs_of_a =
            unsafe { std::mem::transmute::<_, Vec<Limb32>>(a_vec) };
        const_field("FIELD_ED_A", limbs_of_a)
    } else {
        let a_vec = limbs_of_u64(&a).to_vec();
        let limbs_of_a =
            unsafe { std::mem::transmute::<_, Vec<Limb64>>(a_vec) };
        const_field("FIELD_ED_A", limbs_of_a)
    };
    ...
    let d_def = if TypeId::of::<L>() == TypeId::of::<Limb32>() {
        ...
        let limbs_of_d =
            unsafe { std::mem::transmute::<_, Vec<Limb32>>(d_vec) };
        const_field("FIELD_ED_D", limbs_of_d)
    } else {
        let d_vec = limbs_of_u64(&d).to_vec();
        let limbs_of_d =
            unsafe { std::mem::transmute::<_, Vec<Limb64>>(d_vec) };
        const_field("FIELD_ED_D", limbs_of_d)
    };
    ...
    let k_def = if TypeId::of::<L>() == TypeId::of::<Limb32>() {
        ...
        let limbs_of_k =
            unsafe { std::mem::transmute::<_, Vec<Limb32>>(k_vec) };
        const_field("FIELD_ED_K", limbs_of_k)
    } else {
        let k_vec = limbs_of_u64(&k).to_vec();
        let limbs_of_k =
            unsafe { std::mem::transmute::<_, Vec<Limb64>>(k_vec) };
        const_field("FIELD_ED_K", limbs_of_k)
    };
...
fn multiexp_bucket(point: &str, exp: &str) -> String {
    String::from(MULTIEXP_BUCKET_SRC)
}

fn matter_lab_ec(point: &str, exp: &str) -> String {
    String::from(MATTER_LAB_EC_SRC)
}

fn common_cuh(point: &str, exp: &str) -> String {
    String::from(COMMON_CUH_SRC)
}
fn carry_chain(point: &str, exp: &str) -> String {
    String::from(CARRY_CHAIN_SRC)
}
fn ff_config_h(point: &str, exp: &str) -> String {
    String::from(FF_CONFIG_H_SRC)
}
fn ff_dispatch_st(point: &str, exp: &str) -> String {
    String::from(FF_DISPATCH_ST_SRC)
}
fn memory(point: &str, exp: &str) -> String {
    String::from(MEMORY_SRC)
}
fn ptx(point: &str, exp: &str) -> String {
    String::from(PTX_SRC)
}

fn ff_storage(point: &str, exp: &str) -> String {
    String::from(FF_STORAGE)
}
fn host_math(point: &str, exp: &str) -> String {
    String::from(HOST_MATH_SRC)
}

fn zprize_permutation(field: &str) -> String {
    String::from(ZPRIZE_1B_PERMUTATION).replace("FIELD", field)
}
...
pub trait Limb: Sized + Clone + Copy + 'static {
    ...
    /// `Self::LimbType` (least Returns the limbs that represent the
    /// multiplicative identity of the given field.
...
impl Limb for Limb32 {
    ...
    fn one_limbs<F: Field>() -> Vec<Self> {
        ...
        let limbs_of_one =
            unsafe { std::mem::transmute::<_, Vec<Self>>(limbs_of_one) };
    ...
    fn modulus_limbs<F: Field>() -> Vec<Self> {
        ...
        let limbs_of_modulus =
            unsafe { std::mem::transmute::<_, Vec<Self>>(limbs_of_modulus) };
    ...
    fn calculate_r2<F: Field>() -> Vec<Self> {
        let r2 =
            <<<F as ark_ff::Field>::BasePrimeField as PrimeField>::Params>::R2
                .clone();

        let limbs_of_r2 = limbs_of_u32(&r2).to_vec();
        let limbs_of_r2 =
            unsafe { std::mem::transmute::<_, Vec<Self>>(limbs_of_r2) };
...
/// Returns CUDA/OpenCL source-code that contains definitions/functions that are
/// shared across fields.
///
/// It needs to be called before any other function like [`field`] or
/// [`gen_ec_source`] is called, as it contains deinitions, used in those.
