...
use crate::api::{
    cuMemAlloc_v2, cuMemFreeAsync, cuMemFree_v2, cuMemcpy, cuMemcpyAsync,
    cuMemcpyDtoDAsync_v2, cuMemcpyDtoD_v2, cuMemcpyDtoH_v2, cuMemcpyHtoD_v2,
    cuMemcpyPeer, cuMemsetD32Async, CUdeviceptr, CUstream,
};
...
impl<T> DeviceMemory<T> {
    pub fn new(ctx: &CudaContext, size: usize) -> CudaResult<Self> {
        ...
        let result = unsafe {
            cuMemAlloc_v2(&mut mem_ptr as *mut CUdeviceptr, byte_size)
        };
    ...
    pub fn free_async(&mut self, stream: CUstream) -> CudaResult<()> {
        let result = unsafe { cuMemFreeAsync(self.inner, stream) };
        if result == 0 {
            Ok(())
        } else {
            Err(result.into())
    ...
    pub fn reset(&mut self, stream: CUstream) -> CudaResult<()> {
        let result = unsafe { cuMemFreeAsync(self.inner, stream) };
        if result == 0 {
            Ok(())
        } else {
    ...
    pub fn write_to_static(
        ...
        let size_in_bytes = std::mem::size_of::<T>() * size;
    ...
    pub fn memcpy_from_to(
        ...
        let result = unsafe {
            cuMemcpyDtoD_v2(dst.get_inner(), src.get_inner(), size_in_bytes)
        };
    ...
    pub fn memcpy_from_to_raw_async(
        src: CUdeviceptr,
        dst: CUdeviceptr,
        size: u64,
        stream: &CudaStream,
        ...
        let result =
            unsafe { cuMemcpyDtoDAsync_v2(dst, src, size, stream.get_inner()) };
