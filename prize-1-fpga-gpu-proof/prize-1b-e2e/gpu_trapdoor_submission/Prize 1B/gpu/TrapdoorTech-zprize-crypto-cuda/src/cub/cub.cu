                                  ...
                                  unsigned *d_values_out, int num_items, int begin_bit, int end_bit, cudaStream_t stream)
{
  ...
  else
  {
    ...
    ret = DeviceRadixSort::SortPairs(
        d_temp_storage, ret_temp_storage_bytes, d_keys_in, d_keys_out, d_values_in, d_values_out, num_items, begin_bit, end_bit, stream);
  }
                                             ...
                                             const unsigned *d_values_in, unsigned *d_values_out, int num_items, int begin_bit, int end_bit, cudaStream_t stream)
{
  ...
  else
  {
    ...
    ret = DeviceRadixSort::SortPairsDescending(d_temp_storage, ret_temp_storage_bytes, d_keys_in, d_keys_out, d_values_in, d_values_out, num_items, begin_bit, end_bit, stream);
  }
                                         ...
                                         unsigned *d_num_runs_out, int num_items, cudaStream_t stream)
{
  ...
  {
    size_t ret_temp_storage_bytes = *temp_storage_bytes;
    ret = DeviceRunLengthEncode::Encode(d_temp_storage, ret_temp_storage_bytes, d_in, d_unique_out, d_counts_out, d_num_runs_out, num_items, stream);
  }
...
extern "C" cudaError_t exclusive_sum(void *d_temp_storage, size_t *temp_storage_bytes, const unsigned *d_in, unsigned *d_out, int num_items, cudaStream_t stream)
{
  ...
  {
    size_t ret_temp_storage_bytes = *temp_storage_bytes;
    ret = DeviceScan::ExclusiveSum(d_temp_storage, ret_temp_storage_bytes, d_in, d_out, num_items, stream);
  }
