...
/// Since Cuda context is binded to a CPU thread, our kernel must also save
/// the context while being created NOTE: There are several buffer names
/// reserved by kernel: POWERS_BUFFER, RESULT_BUFFER, BUCKET_BUFFER,
/// FFT_PQ_BUFFER, FFT_OMEGAS_BUFFER
use crate::errors::{GPUError, GPUResult};
use crate::{utils::*, CudaFunction};
use crate::{
    Context, Device, DeviceMemory, GPUSourceCore, Module, Params, Stream,
};

use crypto_cuda::{make_params, CUdeviceptr};
...
/// GpuPoly represents a specific buffer space on GPU that can be viewed as a
/// polynomial, it is used for better abstraction NOTE: Dropping a GpuPoly will
/// trigger cuda memory revocation `F` represents the type that we use for gpu
/// computing `T` represents the type that external code uses
...
/// type cast works as long as the underlying representations of data are
/// identical
    ...
    F: PrimeField,
{
    ...
    /// handy functions for creating specialized buffers
    pub fn new_omegas_buffer(
        kern: &'a PolyKernel<'b, F>,
    ) -> GPUResult<GpuPoly<'a, 'b, F>> {
        Ok(GpuPoly::<F>::new(&kern, LOG2_MAX_ELEMENTS)?)
    }

    pub fn new_omegas_buffer_with_size(
        kern: &'a PolyKernel<'b, F>,
        n: usize,
    ) -> GPUResult<GpuPoly<'a, 'b, F>> {
    ...
    pub fn copy_from_gpu_offset(
        ...
        let kernel_name =
            format!("{}_poly_{}", self.fr_name, "copy_from_offset_to");
    ...
    pub fn copy_from_gpu_offset_with_len(
        &mut self,
        src: &GpuPoly<F>,
        offset: usize,
        len: usize,
        ...
        let kernel_name =
            format!("{}_poly_{}", self.fr_name, "copy_from_offset_to");
        let params = make_params!(p2_buf, p1_buf, offset as u32, len as u32);
    ...
    pub fn copy_to_gpu_offset_with_len(
        &self,
        dst: &mut GpuPoly<F>,
        offset: usize,
        len: usize,
    ) -> GPUResult<()> {
        if self.memory.get_inner() == dst.get_memory().get_inner() {
            return Ok(());
        }

        let n = self.size;
        assert!(n >= len);
        assert!(len + offset <= dst.size());

        let src_buf = self.memory.get_inner();
        let dst_buf = dst.get_memory().get_inner();

        let kernel_name =
            format!("{}_poly_{}", self.fr_name, "copy_from_to_offset");
        let params = make_params!(src_buf, dst_buf, offset as u32, len as u32);
    ...
    pub fn add_assign_gpu_ptr(
        &mut self,
        poly_ptr: CUdeviceptr,
        n: usize,
    ) -> GPUResult<()> {
        // TODO: is this correct?
        //assert!(self.size >= poly.size());

        let n = std::cmp::min(self.size, n);
    ...
    pub fn batch_inversion(
        ...
        let spawn = std::cmp::min(
            n,
            self.kernel.get_core_count() * self.kernel.get_batch(),
        );
        let chunk_size = n / spawn + (n % spawn != 0) as usize;

        // keeping inner loop in part_2 small by limiting `bucket_num`, each
        // bucket will have at most `MAX_BUCKET_SIZE` elements
        let bucket_num = std::cmp::min(
            chunk_size / MAX_BUCKET_SIZE
                + (chunk_size % MAX_BUCKET_SIZE != 0) as usize,
            MAX_BUCKET_NUM,
        );
        ...
        let kernel_name =
            format!("{}_poly_{}_part_1", self.fr_name, BatchInversion);
        ...
        let product_inverse = full_grand_product.inverse().ok_or(
            GPUError::Simple("batch inversion division by zero".to_owned()),
        )?;
        ...
        let kernel_name =
            format!("{}_poly_{}_part_2", self.fr_name, BatchInversion);
    ...
    /// calculate powers of g by radix 2, then fill a whole buffer with
    /// sequential powers of `g`
    pub fn generate_powers<T>(
        &mut self,
    ...
    /// Shrink a large domain poly evaluations into small domain poly
    /// evaluations
    ...
    pub fn fill_with_zero(&mut self) -> GPUResult<()> {
        ...
        self.memory
            .memset_async(0u32, n, self.kernel.get_stream())?;
    ...
    pub fn setup_omegas_with_size<T>(
        &mut self,
        omega: &T,
        n: usize,
    ) -> GPUResult<()> {
    ...
    /// Note: after each fft round, the internal DeviceMemory of src/dst buffers
    /// are swapped
    fn radix_fft_round(
        &mut self,
    ...
    /// do a full FFT on self buffer, including computing all the necessary
    /// pq/omegas
    pub fn fft_full<T>(&mut self, omega: &T) -> GPUResult<()> {
        ...
        let mut tmp_buffer = GpuPoly::<F>::new(self.get_kernel(), n)?;

        let mut pq_buffer = GpuPoly::<F>::new_pq_buffer(self.get_kernel())?;

        let mut omegas_buffer =
            GpuPoly::<F>::new_omegas_buffer_with_size(self.get_kernel(), n)?;
    ...
    /// do a full iFFT on self buffer, including computing all the necessary
    /// pq/omegas
    pub fn ifft_full<T>(&mut self, omega_inv: &T) -> GPUResult<()> {
        ...
        let size_inv =
            F::from_str(&format!("{}", self.size)).map_err(|_| {
                GPUError::Simple(
                    "cannot get minv: incorrect field element".to_owned(),
                )
            })?;
