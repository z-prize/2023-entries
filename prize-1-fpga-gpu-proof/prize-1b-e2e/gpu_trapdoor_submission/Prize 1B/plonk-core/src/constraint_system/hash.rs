    ...
    P: TEModelParameters<BaseField = F>,
{
    ...
    ///
    pub fn full_affine_transform_gate_2(
        &mut self,
        vars: &[Variable; 3],
        selectors: &[F; 5],
        index: &mut usize,
        ...
        let w4_var = self.add_input_2(w4_val, variable_index, reuse_perm);

        // add wires
        // self.w_l.push(vars[0]);
        self.w_l[*index] = vars[0];
        // self.w_r.push(vars[1]);
        self.w_r[*index] = vars[1];
        // self.w_o.push(w4_var);
        self.w_o[*index] = w4_var;
        // self.w_4.push(vars[2]);
        self.w_4[*index] = vars[2];

        // add high degree selectors
        // self.q_hl.push(selectors[0]);
        self.q_hl[*index] = selectors[0];
        // self.q_hr.push(selectors[1]);
        self.q_hr[*index] = selectors[1];
        // self.q_h4.push(selectors[2]);
        self.q_h4[*index] = selectors[2];
        // self.q_c.push(selectors[3]);
        self.q_c[*index] = selectors[3];
        // self.q_o.push(selectors[4]);
        self.q_o[*index] = selectors[4];
        // add dummy selector vectors
        // self.q_l.push(F::zero());
        ...
        //         vars[0], vars[1], w4_var, vars[2], *index,
        //     );
        // }
        // self.n += 1;
        // println!("full_affine_transform_gate 1 spent:{:?}", start.elapsed());
        *index += 1;
        self.n = *index;
        w4_var
    }
    ...
    ///
    pub fn partial_affine_transform_gate_2(
        &mut self,
        vars: &[Variable; 3],
        selectors: &[F; 5],
        index: &mut usize,
        ...
        self.q_l[*index] = F::zero();
        // self.q_r.push(selectors[1]);
        self.q_r[*index] = selectors[1];
        // self.q_m.push(F::zero());
        self.q_m[*index] = F::zero();
        // self.q_4.push(selectors[2]);
        self.q_4[*index] = selectors[2];
        // self.q_arith.push(F::one());
        self.q_arith[*index] = F::one();
        ...
        // self.perm
        //     .lock().unwrap().add_variables_to_map(vars[0], vars[1], w4_var,
        // vars[2], *index); self.n += 1;
        // if !reuse_perm {
        //     perm.add_variables_to_map(
        //         vars[0], vars[1], w4_var, vars[2], *index,
        //     );
