...
fn main() {
    ...
    // ==============================
    {
        ...
        println!("## pp time is {:?}", now.elapsed());
        ...
        // proprocessing
        let proprocess_time = std::time::Instant::now();
        let (pk, (vk, _pi_pos)) =
            dummy_circuit.compile::<KZG10<Bls12_381>>(&pp).unwrap();
        println!("## proprocess time is {:?}", proprocess_time.elapsed());
        ...
        let (proof, pi) = {
            ...
            println!("## ..prover new time is {:?}", time1.elapsed());
            let gadget_time = std::time::Instant::now();
            real_circuit.gadget(prover.mut_cs()).unwrap();
            println!("## ..gadget time is {:?}", gadget_time.elapsed());
            let proof_gen_time = std::time::Instant::now();
            real_circuit
                .gen_proof::<KZG10<Bls12_381>>(&pp, pk, b"Merkle tree")
                .unwrap()
        };
        println!("## proof calculation time is {:?}", proof_cal.elapsed());
        println!("proof generation time is {:?}", now.elapsed());
