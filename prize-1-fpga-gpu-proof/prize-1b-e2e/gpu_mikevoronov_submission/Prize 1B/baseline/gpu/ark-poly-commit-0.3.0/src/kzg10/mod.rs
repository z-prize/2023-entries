...
use ark_std::{format, log2, marker::PhantomData, ops::Div, vec};
...
use ec_gpu_common::{gpu_msm, G1Affine, MSMContext, MultiexpKernel};
use std::mem::transmute;

use ec_gpu_common::api::get_commit;
    ...
    for<'a, 'b> &'a P: Div<&'b P, Output = P>,
{
    ...
    fn powers_of_2(
        bases_size: usize,
        scalars: &mut Vec<<<E as PairingEngine>::Fr as PrimeField>::BigInt>,
    ) {
        let scalars_len = scalars.len();
        let size = ark_std::cmp::min(bases_size, scalars_len);

        let log_count = log2(size);
        let size = 1 << log_count;

        assert!(size <= bases_size);

        if size > scalars_len {
            scalars.resize(
                size,
                <<E as PairingEngine>::Fr as PrimeField>::BigInt::from(0),
            );
        }
    }

    pub fn get_gpu_context<'a, 'b, 'c, G>(
    ...
    where
        ...
        G: AffineCurve,
    {
        let bases = powers.powers_of_g.to_vec();
        assert!(coeffs_count <= bases.len());

        let bases = &bases[..coeffs_count];
        let bases = unsafe { transmute::<_, &[G1Affine]>(bases) };
        msm_kern.multi_scalar_mult_init(bases, msm_kern)
    }
    
    pub fn get_commitment<'a, 'b, G>(
         point: i32
    ) -> Commitment<E>
    where
        'a: 'b,
        G: AffineCurve
    {        
        let mut from = vec![<E as PairingEngine>::G1Projective::zero(); 1];
		let mut from_aff = vec![<E as PairingEngine>::G1Affine::zero(); 1];
		let mut result = <E as PairingEngine>::G1Affine::zero();
        if (point >= 0) {
            let is_affine = unsafe { get_commit(point, from.as_mut_ptr() as *mut c_void, from_aff.as_mut_ptr() as *mut c_void) };
			if (is_affine == 0) {
				result = from[0].into();
			}
			else {
    ...
    where
        G: AffineCurve,
    {
        //println!("size = {0}", mem::size_of::<Commitment<E>>());
        ...
        let (num_leading_zeros, mut plain_coeffs) = skip_leading_zeros_and_convert_to_bigints(polynomial);
        assert!(num_leading_zeros == 0);

        let msm_time = start_timer!(|| "MSM to compute commitment to plaintext poly");

        let mut results = vec![];
        if plain_coeffs.len() < 1 << 4 || msm_context.is_none() {
            ...
            Self::powers_of_2(1 << msm_context.unwrap().get_log_count(), &mut plain_coeffs);

            let scalars = unsafe { transmute::<_, &Vec<BigInteger256>>(&plain_coeffs) };
            let commitment_gpu = gpu_msm(scalars, msm_context.unwrap());

            let commitment_gpu = unsafe {transmute::<_, &<E as PairingEngine>::G1Projective>(&commitment_gpu) }; 

            results.push(*commitment_gpu);
            /*if true {
                let commitment = VariableBaseMSM::multi_scalar_mul(
                    &powers.powers_of_g[num_leading_zeros..],
                    &plain_coeffs,
                );
                assert_eq!(commitment, commitment_gpu.clone());
            }*/
            //println!("after msm {:?}", results.as_slice()[0].into());
        }
        end_timer!(msm_time);

        
        let mut commitment = results.as_slice()[0];
        ...
        /*if (msm_context.is_none()) {
        }
        else {
            println!("len = {0}",randomness.blinding_polynomial.coeffs().len());
        }*/

        let random_ints: Vec<<<E as PairingEngine>::Fr as PrimeField>::BigInt> =
    ...
    pub fn compute_witness_polynomial(
        ...
        /*let last = witness_polynomial.coeffs().len() - 1;
        println!("{:?}", witness_polynomial.coeffs()[0]);
        println!("{:?}", witness_polynomial.coeffs()[1]);
        println!("{:?}", witness_polynomial.coeffs()[2]);
        println!("{:?}", witness_polynomial.coeffs()[3]);
        println!("{:?}\n", witness_polynomial.coeffs()[last]);*/
    ...
    pub(crate) fn open_with_witness_polynomial<'a, 'b, G>(
        powers: &Powers<E>,
        point: P::Point,
        randomness: &Randomness<E::Fr, P>,
        ...
        G: AffineCurve,
    {
        ...
        let (num_leading_zeros, mut witness_coeffs) = skip_leading_zeros_and_convert_to_bigints(witness_polynomial);

        let witness_comm_time = start_timer!(|| "Computing commitment to witness polynomial");
        let w;
        //TODO
        if (witness_coeffs.len() < 1 << 4 || msm_context.is_none()) {
            w = VariableBaseMSM::multi_scalar_mul(&powers.powers_of_g[num_leading_zeros..], &witness_coeffs);
        }
        else {
            
            Self::powers_of_2(1 << msm_context.unwrap().get_log_count(), &mut witness_coeffs);

            let scalars = unsafe { transmute::<_, &Vec<BigInteger256>>(&witness_coeffs) };
            let commitment_gpu = gpu_msm(scalars, msm_context.unwrap());
            w = unsafe {*transmute::<_, &<E as PairingEngine>::G1Projective>(&commitment_gpu) };
        }
