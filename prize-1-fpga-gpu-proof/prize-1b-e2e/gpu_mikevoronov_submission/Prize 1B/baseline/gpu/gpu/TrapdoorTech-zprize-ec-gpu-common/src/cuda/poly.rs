    ...
    F: PrimeField,
{
    ...
    pub fn batch_inversion(
        ...
        println!("sp {:?}", spawn);
        println!("cores {:?}", self.kernel.get_core_count());
        println!("gt {:?}", self.kernel.get_batch());
        println!("chunk_size {:?}", chunk_size);
        println!("bucket_num {:?}", bucket_num);
    ...
    pub fn fft_full<T>(&mut self, omega: &T) -> GPUResult<()> {
        ...
        let mut omegas_buffer = GpuPoly::<F>::new_omegas_buffer_with_size(self.get_kernel(), n)?;
