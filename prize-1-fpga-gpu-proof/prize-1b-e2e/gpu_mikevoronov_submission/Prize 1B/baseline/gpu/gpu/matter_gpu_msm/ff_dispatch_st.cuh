...
template <class FF_CONFIG, const uint32_t INV> struct ff_dispatch_st {
  ...
  static constexpr HOST_DEVICE_INLINE storage get_zero() { return CONFIG::zero; }
  static constexpr HOST_DEVICE_INLINE storage get_m_one() { return CONFIG::m_one; }
  
  static constexpr HOST_DEVICE_INLINE storage get_k1() { return CONFIG::k1; }
  static constexpr HOST_DEVICE_INLINE storage get_k2() { return CONFIG::k2; }
  static constexpr HOST_DEVICE_INLINE storage get_k3() { return CONFIG::k3; }
...
#pragma unroll
  ...
  template <unsigned REDUCTION_SIZE = 1> static constexpr HOST_DEVICE_INLINE storage reduce(const storage &xs) {
    ...
    // printf("reduce  moduls [0]:%x, [7]:%x, REDUCTION_SIZE:%d\n", modulus.limbs[0], modulus.limbs[7], REDUCTION_SIZE);
    storage rs = {};
...
#pragma unroll
  ...
  template <unsigned REDUCTION_SIZE> static constexpr DEVICE_INLINE storage mul_device(const storage &xs, const storage &ys) {
    // Forces us to think more carefully about the last carry bit if we use a modulus with fewer than 2 leading zeroes of slack
    // printf(" mul device  , REDUCTION_SIZE:%d\n", REDUCTION_SIZE);
    // static_assert(!(CONFIG::modulus.limbs[TLC - 1] >> 30));

    static_assert(!(CONFIG::modulus.limbs[TLC - 1] >> 31));
    storage rs = {0};
  ...
  template <unsigned REDUCTION_SIZE> static constexpr DEVICE_INLINE storage sqr_device(const storage &xs) {
    // TODO: Forces us to think more carefully about the last carry bit if we use a modulus with fewer than 2 leading zeroes of slack    
    //static_assert(!(CONFIG::modulus.limbs[TLC - 1] >> 30));
    storage_wide rs = {0};
    sqr_raw(xs, rs);
  ...
  template <unsigned REDUCTION_SIZE> static constexpr HOST_INLINE storage mul_host(const storage &xs, const storage &ys) {
    ...
    //printf("mul host --------[0]:%x, %x, %x, REDUCTION_SIZE:%d, q:%x,  x:[0]:%x, tlc:%d\n", n[0], n[1], n[11], REDUCTION_SIZE, q, x[0], TLC);
...
//TODO: need to add pow function
  template <unsigned REDUCTION_SIZE = 1> static constexpr HOST_DEVICE_INLINE storage pow5(const storage &xs) {
#ifdef __CUDA_ARCH__
    storage x2 = mul_device<REDUCTION_SIZE>(xs, xs); //x2
    storage x4 = mul_device<REDUCTION_SIZE>(x2, x2); //x4
    return mul_device<REDUCTION_SIZE>(x4, xs);
#else
    storage x2 = mul_host<REDUCTION_SIZE>(xs, xs); //x2
    storage x4 = mul_host<REDUCTION_SIZE>(x2, x2); //x4
    return mul_host<REDUCTION_SIZE>(x4, xs);
#endif
  }
...
#pragma unroll
  ...
  static constexpr DEVICE_INLINE uint32_t extract_bits(const storage &xs, const unsigned offset, const unsigned count) {
    ...
    // unsigned int ret;
    // asm volatile("shf.r.wrap.b32 %0, %1, %2, %3;" : "=r"(ret) : "r"(low_limb), "r"(high_limb), "r"(offset));
    // return ret;
    // uint32_t result = ret;
