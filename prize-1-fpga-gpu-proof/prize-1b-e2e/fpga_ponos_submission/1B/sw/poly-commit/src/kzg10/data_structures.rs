...
pub struct ExtPowers<'a, E: PairingEngine> {
    /// Group elements of the form `β^i G`, for different values of `i`.
    pub powers_of_g: Cow<'a, [E::G1Affine]>,
    /// In normal coordinates
    pub n_powers_of_g: Cow<'a, [ponos_engine::NBase384]>,

    /// Group elements of the form `β^i γG`, for different values of `i`.
    pub powers_of_gamma_g: Cow<'a, [E::G1Affine]>,
    /// In normal coordinates
    pub n_powers_of_gamma_g: Cow<'a, [ponos_engine::NBase384]>,
}

impl<E: PairingEngine> ExtPowers<'_, E> {
    /// The number of powers in `self`.
    pub fn size(&self) -> usize {
        self.powers_of_g.len()
    }
