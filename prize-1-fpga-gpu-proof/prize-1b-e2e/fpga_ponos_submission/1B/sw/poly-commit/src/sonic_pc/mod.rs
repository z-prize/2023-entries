    ...
    for<'a, 'b> &'a P: Div<&'b P, Output = P>,
{
    ...
    fn trim(
        ...
        let n_powers_of_g = ponos_engine::to_normal_vec(powers_of_g.as_slice());
        let n_powers_of_gamma_g = ponos_engine::to_normal_vec(powers_of_gamma_g.as_slice());
        let n_shifted_powers_of_g = shifted_powers_of_g.as_ref().map(|a| ponos_engine::to_normal_vec(a.as_slice()));
        let n_shifted_powers_of_gamma_g = shifted_powers_of_gamma_g.as_ref().map(|a| a.into_iter().map(|(k, v)| (*k, ponos_engine::to_normal_vec(v.as_slice()))).collect::<BTreeMap<_, _>>());
        ...
        let ck = CommitterKey {
            ...
            n_powers_of_gamma_g,
            shifted_powers_of_g,
            n_shifted_powers_of_g,
            shifted_powers_of_gamma_g,
            n_shifted_powers_of_gamma_g,
        ...
        P: 'a,
    {
        ...
        for labeled_polynomial in polynomials {
            ...
            let ext_powers = if let Some(degree_bound) = degree_bound {
                ck.ext_shifted_powers(degree_bound).unwrap()
            } else {
                ck.ext_powers()
            };

            let (comm, rand) = kzg10::KZG10::commit(&ext_powers, polynomial, hiding_bound, Some(rng))?;
