...
pub struct CommitterKey<E: PairingEngine> {
    ...
    /// The key used to commit to polynomials.
    pub n_powers_of_g: Vec<ponos_engine::NBase384>,

    /// The key used to commit to hiding polynomials.
    pub powers_of_gamma_g: Vec<E::G1Affine>,
    /// The key used to commit to hiding polynomials.
    pub n_powers_of_gamma_g: Vec<ponos_engine::NBase384>,
    ...
    /// The powers used to commit to shifted polynomials.
    /// This is `None` if `self` does not support enforcing any degree bounds.
    pub n_shifted_powers_of_g: Option<Vec<ponos_engine::NBase384>>,
    ...
    /// The powers used to commit to shifted hiding polynomials.
    /// This is `None` if `self` does not support enforcing any degree bounds.
    pub n_shifted_powers_of_gamma_g: Option<BTreeMap<usize, Vec<ponos_engine::NBase384>>>,
...
impl<E: PairingEngine> CommitterKey<E> {
    ...
    /// Obtain powers for the underlying KZG10 construction
    pub fn ext_powers(&self) -> kzg10::ExtPowers<E> {
        kzg10::ExtPowers {
            powers_of_g: self.powers_of_g.as_slice().into(),
            n_powers_of_g: self.n_powers_of_g.as_slice().into(),
            powers_of_gamma_g: self.powers_of_gamma_g.as_slice().into(),
            n_powers_of_gamma_g: self.n_powers_of_gamma_g.as_slice().into(),
        }
    }
    ...
    /// Obtain powers for committing to shifted polynomials.
    pub fn ext_shifted_powers(
        &self,
        degree_bound: impl Into<Option<usize>>,
    ) -> Option<kzg10::ExtPowers<E>> {
        match (&self.shifted_powers_of_g, &self.shifted_powers_of_gamma_g) {
            (Some(shifted_powers_of_g), Some(shifted_powers_of_gamma_g)) => {
                let max_bound = self
                    .enforced_degree_bounds
                ...
                let ck = kzg10::ExtPowers {
                    powers_of_g: shifted_powers_of_g[powers_range.clone()].into(),
                    n_powers_of_g: self.n_shifted_powers_of_g.as_ref().unwrap()[powers_range.clone()].into(),
                    powers_of_gamma_g: shifted_powers_of_gamma_g[&bound].clone().into(),
                    n_powers_of_gamma_g: self.n_shifted_powers_of_gamma_g.as_ref().unwrap()[&bound].clone().into(),
                };
